...
MAX_RETRIES=5                       
RETRY_DELAY=30                      
APP_FILE="manager_fastapi.py"  # Name of the FastAPI manager app file

# Check if all IP parts are provided
if [ -z "$1" ] || [ -z "$2" ] || [ -z "$3" ] || [ -z "$4" ]; then
  echo "Usage: ./setup_fastapi_manager.sh <part1> <part2> <part3> <part4>"
  exit 1
fi
...
# Upload the FastAPI manager app file
echo "Uploading the FastAPI manager app file..."
...
ssh -i "$KEY_PATH" -o StrictHostKeyChecking=no $REMOTE_USER@${INSTANCE_IP} << EOF
    ...
    mkdir -p ~/fastapi_manager
    mv ~/manager_fastapi.py ~/fastapi_manager/

    # Create a systemd service file for FastAPI
    echo "[Unit]
    Description=FastAPI manager Service
    After=network.target

    [Service]
    User=${REMOTE_USER}
    WorkingDirectory=/home/${REMOTE_USER}/fastapi_manager
    ExecStart=/home/${REMOTE_USER}/fastapi_env/bin/uvicorn manager_fastapi:app --host 0.0.0.0 --port 8000
    Restart=always

    [Install]
    WantedBy=multi-user.target" | sudo tee /etc/systemd/system/fastapi_manager.service

    # Reload systemd and start the FastAPI service
    sudo systemctl daemon-reload
    sudo systemctl enable fastapi_manager
    sudo systemctl start fastapi_manager

    echo "FastAPI manager service has been successfully deployed and started."
EOF

echo "FastAPI manager setup completed on ${INSTANCE_IP}."
